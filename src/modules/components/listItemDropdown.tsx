...
const ListItemDropdown = (props: ListItemProps) => {
    ...
    return <>
        <button onClick={handleClick} className="hover:translate-x-1 py-3 flex text-gray-500 w-full hover:text-primary-500 duration-300 items-center justify-between">
            <a className="pl-2.5 pr-0.5 justify-center items-center">{icon}</a>
            <h1 className="absolute pl-9 invisible lg:visible font-medium">{title}</h1>
            <a className={`${isRotated ? 'rotate-180' : ''} transition duration-200 mr-3 `}><FiChevronDown /></a>
        </button>
        {showOptions && (
